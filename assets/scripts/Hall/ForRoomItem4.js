...
cc.Class({
    ...
    OnLoaded (params) {
        ...
        {
            this.node.x = params.startX + itmWidth/2;
        }else
        {
            this.node.x = params.startX + this.param.number * (itmWidth) + itmWidth/2 + 10 * this.param.number;
        }
    ...
    OnForRoom_Invite:function()
    {
        ...
        if (gameDef.ConfigString)
        {
            ...
            // var title = gameStr + ProGameRuleConfig.gameRound + "局," + this.toChinese(this.param.user_num) + "缺" + this.toChinese(this.param.desk_seat - this.param.user_num) + "。";
            var title = gameStr + ProGameRuleConfig.gameRound + "局," + "房号:" + sRoomNum + ".";          
