...
function _Utf8ToUnicode(strUtf8) {
	var i,j;
	var uCode;
	var temp = new Array();
...
function _GetMd5EncryptStr(paramsList) {

	var paramArray=[];
	for (const key in paramsList) {
		paramArray.push(paramsList[key]);
	}
	paramArray.sort();
	var paramStr = paramArray.join("");
	var md5Str = md5Encode(paramStr,"");
	md5Str = md5Str.toLowerCase();
	paramsList.sign = md5Str;
	var paramStr_md5 = "";
	for (const key in paramsList) {
		if (""!=paramStr_md5) {
			paramStr_md5 = paramStr_md5 + "&";
		}
		paramStr_md5 = paramStr_md5 + key + "=" + paramsList[key];
	}
	return paramStr_md5;
};
...
function _ShowTooltip(msgStr){
    cc.loader.loadRes( "Hall/Node_Tooltip", function( error, perfab ) {
        if( error ){
...
function sort( arr,type = 0)
{
    ...
    for (var i = 0; i < num; i++)
    {
        ...
        {
            if (arr[i] != undefined && arr[j] != undefined && arr[i] != null && arr[j] != null) 
            {
                var a = arr[i];
                var b = arr[j];
                if(type === 0)
...
{
    return node.runAction(cc.sequence(cc.delayTime(time),cc.callFunc(callback)));
}

function MySchedule(node,callback,time)
{
...
function Unschedule(node,timer)
{
    node.stopAction(timer);
}

function getTime()
{
    var myData = new Date();
    var strTime = MytoString(myData.getMonth() + 1,2) + "月" + MytoString(myData.getDate(),2) + "日 " + MytoString(myData.getHours(),2) + ":" + MytoString(myData.getMinutes(),2);
    return strTime;
}

function MytoString(num,i)
{
...
function SetClipboardStr(str)
{
    if( typeof wx != "undefined" )
    {
        var dataObj = {data:str}
        wx.setClipboardData(dataObj);
    }
}

module.exports = {
    UnicodeToUtf8 : _UnicodeToUtf8,
	Utf8ToUnicode : _Utf8ToUnicode,
	GetMd5EncryptStr : _GetMd5EncryptStr,
    ...
    remove:remove,
    resetArr:resetArr,
	SetClipboardStr:SetClipboardStr,
};